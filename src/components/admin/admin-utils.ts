...
import { AllModuleDataTypes, QuerySpec } from '../../types';
import { KeyedModuleSpec } from './module-list-utils';
import { SECTION_TYPES, MODULE_LABELS , CATEGORY_COLOR_MAP } from '@/constants';
...
export function queryObj2Str(query?: object): string {
  ...
  return Object.entries(query).reduce(
    (str, [key, value], idx) =>
      str + (idx === 0 ? '' : '&') + (typeof value == 'undefined' ? key : `${key}=${value}`),
    ''
  );
}
